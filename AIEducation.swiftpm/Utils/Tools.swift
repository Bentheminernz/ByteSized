...
struct WeatherTool: Tool {
  ...
  let description = "Get the current weather conditions for a specified city including temperature, condition, and humidity."
  ...
  private let mockWeatherData: [String: (temp: Double, condition: String, humidity: Double, wind: Double)] = [
  ...
  func call(arguments: Arguments) async throws -> GeneratedContent {
    ...
    let conditions = ["Sunny", "Partly Cloudy", "Cloudy", "Rainy", "Clear", "Overcast"]
...
struct ContactsTool: Tool {
  ...
  let description = "Search for contacts by name and retrieve their contact information including phone numbers and email addresses."
  ...
  func call(arguments: Arguments) async throws -> GeneratedContent {
    ...
    guard try await ensureContactsAccess() else {
      return GeneratedContent("Contact access is not authorized. Please enable contacts access in Settings.")
    }
    
    do {
      let contacts = try fetchContacts(matching: arguments.name, limit: arguments.limit ?? 5)
      
      if contacts.isEmpty {
        return GeneratedContent("No contacts found matching '\(arguments.name)'.")
      }
      ...
      let header = "Found \(contacts.count) contact(s) matching '\(arguments.name)':\n\n"
      return GeneratedContent(header + formatted)
      
    } catch {
      return GeneratedContent("Error searching contacts: \(error.localizedDescription)")
    }
  }
  ...
  private func fetchContacts(matching name: String, limit: Int) throws -> [CNContact] {
    let keysToFetch: [CNKeyDescriptor] = [
      CNContactGivenNameKey, CNContactMiddleNameKey, CNContactFamilyNameKey, CNContactNamePrefixKey,
      CNContactNameSuffixKey, CNContactNicknameKey, CNContactPhoneticGivenNameKey,
      CNContactPhoneticMiddleNameKey, CNContactPhoneticFamilyNameKey, CNContactOrganizationNameKey,
      ...
      CNContactInstantMessageAddressesKey, CNContactSocialProfilesKey, CNContactRelationsKey
  ...
  private func formatContact(_ contact: CNContact, number: Int) -> String {
    ...
    sections.append(contentsOf: [
      formatLabeledValues(contact.phoneNumbers, title: "Phone Numbers") { $0.stringValue },
      formatLabeledValues(contact.emailAddresses, title: "Email Addresses") { $0 as String },
      formatAddresses(contact.postalAddresses),
      formatLabeledValues(contact.urlAddresses, title: "Websites") { $0 as String },
  ...
  private func formatAddresses(_ addresses: [CNLabeledValue<CNPostalAddress>]) -> String? {
  ...
  private func formatSocialProfiles(_ profiles: [CNLabeledValue<CNSocialProfile>]) -> String? {
    guard !profiles.isEmpty else { return nil }
    
    let lines = profiles.map { profile in
      let service = profile.value.service ?? "Unknown"
  ...
  private func formatInstantMessages(_ messages: [CNLabeledValue<CNInstantMessageAddress>]) -> String? {
    guard !messages.isEmpty else { return nil }
    
    let lines = messages.map { im in
      let service = im.value.service ?? "Unknown"
  ...
  private func formatDates(_ dates: [CNLabeledValue<NSDateComponents>]) -> String? {
    ...
    let lines = dates.compactMap { date -> String? in
      ...
      components.day = day
      guard let dateObj = Calendar.current.date(from: components) else { return nil }
  ...
  private func formatRelations(_ relations: [CNLabeledValue<CNContactRelation>]) -> String? {
    guard !relations.isEmpty else { return nil }
    
    let lines = relations.map { relation in
      let label = CNLabeledValue<NSString>.localizedString(forLabel: relation.label ?? "")
...
struct CalendarTool: Tool {
  ...
  let description = "Fetch calendar events within a specified date range. Can retrieve upcoming events, past events, or events within a custom date range."
  ...
  func call(arguments: Arguments) async throws -> GeneratedContent {
    ...
    guard try await ensureCalendarAccess() else {
      return GeneratedContent("Calendar access is not authorized. Please enable calendar access in Settings.")
    }
    ...
    do {
      ...
      // Format output
      let formatted = formatEvents(limitedEvents, startDate: startDate, endDate: endDate)
      return GeneratedContent(formatted)
      
    } catch {
      return GeneratedContent("Error fetching calendar events: \(error.localizedDescription)")
    }
  }
  ...
  private func determineDateRange(_ arguments: Arguments) throws -> (Date, Date) {
    ...
    if let startDateStr = arguments.startDate {
      guard let endDateStr = arguments.endDate else {
        throw NSError(
          domain: "CalendarTool",
          code: 1,
          userInfo: [NSLocalizedDescriptionKey: "endDate is required when startDate is provided"]
        )
      }
      ...
      guard let start = formatter.date(from: startDateStr) else {
        throw NSError(
          domain: "CalendarTool",
          code: 2,
          userInfo: [NSLocalizedDescriptionKey: "Invalid startDate format. Use ISO 8601 (e.g., 2024-01-15)"]
        )
      }
      ...
      guard let end = formatter.date(from: endDateStr) else {
        throw NSError(
          domain: "CalendarTool",
          code: 3,
          userInfo: [NSLocalizedDescriptionKey: "Invalid endDate format. Use ISO 8601 (e.g., 2024-01-20)"]
        )
      }
    ...
    if days >= 0 {
      ...
      // Looking backward
      let startDate = calendar.date(byAdding: .day, value: days, to: startOfToday)!
      let endOfToday = startOfToday.addingTimeInterval(86400 - 1)
      return (startDate, endOfToday)
    }
  }
  
  private func fetchEvents(from startDate: Date, to endDate: Date, calendarName: String?) -> [EKEvent] {
  ...
  private func formatEvents(_ events: [EKEvent], startDate: Date, endDate: Date) -> String {
    let dateFormatter = DateFormatter()
    dateFormatter.dateStyle = .medium
    dateFormatter.timeStyle = .none
    
    var output = "📅 Calendar Events (\(dateFormatter.string(from: startDate)) - \(dateFormatter.string(from: endDate)))\n"
